...
`myTestApp` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngBomyTestAppcture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngBoilermyTestAppit to manage its codebase. While in theory you don't have
to use Git once you download `ngBoilerplatmyTestAppect makes the assumption
...
While `ngBoilerplate` mmyTestAppe of Node.js behind the scenes, you as the
...
you first start with `ngBoilerplate` you hmyTestAppPM to install the
...
Don't worry about knowing Node.js in order to use `ngBoilerplate`; Grunt ismyTestAppgic happens.
...
to get started with `ngBoilerplate`. But for thosmyTestApplooking to go a
...
Grunt is the engine behind `ngBoilerplate`. It's the magic tmyTestAppmove.
...
`ngBoilerplate` comes with a `bower.jmyTestAppt looks something like this:
